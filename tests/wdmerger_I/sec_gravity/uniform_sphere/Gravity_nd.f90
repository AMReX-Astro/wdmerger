...
! ::
      ...
      !
      ! The strategy for calculating the potential is to first find the
      ! potential at the center of the radial distribution:
      !
      !     phi(0) = int( G * dm / r )
      !
      ! This works if the upper bound is taken to be infinity, and we use
      ! the convention that the potential diminishes towards zero.
      ! Then, the potential in all other zones can be found using
      !
      !     d(phi)/dr = g     ==>    phi(r) = phi(0) + int(g * dr)
      !
      ! where the bounds of the integral are from 0 to r.
      !
