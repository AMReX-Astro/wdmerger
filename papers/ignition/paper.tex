...
range 100--500 km for the grid codes, and 10--100 km for the SPH codes, and observed
...
\added{There is mixed evidence for simulation convergence presented in these studies.
\cite{raskin:2010} claim that their simulations are converged in nickel yield up to 2 million
(constant mass) particles (Figure 11 in that paper), but the nickel yield still appears to be
trending slightly upward with particle count. The simulations from \cite{raskin:2009} are not
converged up to 800,000 particles (Figure 4 in that paper), where the smoothing length was kept
constant instead of the particle mass. \cite{hawley:2012} do not achieve convergence over a
factor of 2 in spatial resolution (Figure 8 in that paper). \cite{garcia-senz:2013} claim at
least qualitative (though not strict absolute) convergence in Section 3.2 of their paper, but
their convergence test is only over a factor of 2 in particle count, which is a factor of 1.26
...
other code (FLASH). \cite{papish:2015} claim convergence in nuclear burning up to 10\% at a
resolution of 5--10 km, but do not present specific data demonstrating this claim.
\cite{loren-aguilar:2010} and \cite{rosswog:2009} do not present convergence studies for
their work.}
...
\citep{seitenzahl:2009,garg:2017} but is generally not larger than 10 km. \added{\citeauthor{kushnir:2013}
...
which was used in both their work and the simulations of \cite{papish:2015}. We will investigate
...
\replaced{Our goal is to record our thoughts on why we believe we did not achieve convergence.}
{We believe the simulations presented below shed light onto why we and others had difficulty
...
\deleted{We note that the WD simulations that inspired this were pure carbon/oxygen,
...
\added{We also tested a similar configuration made of pure carbon/oxygen material (equal fraction by mass).
...
C/O burning. The results for this case are shown in \autoref{fig:self-heat-distance-co}. In this case, the
ignition is central at all resolutions, but the simulation is still clearly unconverged at resolutions worse
than 100 m, as the ignition is prompt at low resolution and delayed at high resolution.}

This story contains two important lessons. First, the required resolution for even a qualitatively converged
simulation, less than 100 m, is out of reach \replaced{of current 3D supernova simulations}{for an analogous
simulation done in 3D}. Second, the behavior for 
...
reacting hydrodynamics simulations. \added{With that being said, our 1D tests are not directly comparable to
...
we could achieve \added{for the test including helium}, about 50 cm, $\tau_s / \tau_e$ was 0.8 at ignition, which is not
...
resolution better than 100 m should \deleted{cast significant uncertainty on the results
of simulations featuring detonations that do not meet this resolution,
regardless of the specific conditions. Or, at least, it should} suggest
that stronger demonstrations of convergence are required \added{for simulations that
do not meet this resolution}. \replaced{especially} {This is especially true} bearing in mind
our observation that the numerical instability can result in simulations that
appear qualitatively converged when the resolution is increased by a factor of
one or two orders of magnitude but not three orders of magnitude.
