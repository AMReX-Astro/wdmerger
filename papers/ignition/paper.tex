...
are not resolved, so it is important to establish \deleted{an understanding of} the
...
detonations are \replaced{very likely not resolved}{possibly not converged}
...
thus set up conditions ripe for a thermonuclear detonation. Since they are easy to set up \added{in a simulation},
...
hawley:2012,garcia-senz:2013} typically had effective spatial resolutions \added{in the burning region of}
\deleted{range} 100--500 km for the grid codes, and 10--100 km for the SPH codes, and observed
...
their convergence test is only over a factor of 2 in particle count, which is a factor of 1.3
...
which was used in their work and later the simulations of \cite{papish:2015}. We will investigate
...
occurred. In practice, we \deleted{will} quantify this requirement as:
\begin{equation}
...
that \replaced{virtually}{almost} all of the energy release occurs in a single timestep even \replaced{if}{though} the
...
sufficiently small to be confident of numerical stability \deleted{at that resolution}. Note
...
these characteristics, the zone should \replaced{detonate}{ignite}.
...
it would likely \replaced{detonate}{ignite}. If the results do not match what occurs at higher
