...
converged thermonuclear ignitions. \added{The inspiration for this work comes from our
...
\added{We note that the WD simulations that inspired this were pure carbon/oxygen,
...
$4 \times 10^9$ K, which we call a thermonuclear ignition (\added{for reference, the
...
low resolution (\added{see \autoref{fig:self-heat-ts_te}}). This means that an ignition is
very likely to occur for numerical reasons, regardless of whether it would occur for physical reasons.
\added{At low resolution, adding more resolution does not meaningfully improve the ratio of
...
than 1 km. \replaced{This does not represent all such burning problems: for example, we
have tested similar setups with only carbon and oxygen for which a resolution
in the tens of kilometers is sufficient.}{This test does not, of course, represent
all possible WD burning conditions.}
However, the fact that it is even possible for
...
\added{regardless of the specific conditions). Or, at least, it should suggest
