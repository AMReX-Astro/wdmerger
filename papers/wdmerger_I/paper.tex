...
the adaptive-mesh refinement approach. In this paper we describe our numerical 
...
confidence in this progenitor model. \MarginPar{Direct numerical
  simulation has a specific meaning to CFD community.  That is the
  simulation resolve the viscous scale.  So maybe we should remove the
word 'direct'.}
...
evaluate the work done at each zone face. However, this application of the divergence theorem is 
mathematically incorrect, despite the fact that this is similar in spirit to the approach discussed here. 
Moreover, using the divergence theorem to integrate the work done in a zone can never obtain 
the factor of $1/2$ corresponding to the change in gravitational self-energy. That factor can be thought of
as a double-counting factor that ensures that we correctly capture the energy of a pairwise interaction. 
When one integrates the work done in the zone, there is a contribution to the energy that cannot be 
expressed as a full time derivative or the divergence of a flux. This reflects the 
global nature of the gravitational potential and means that we cannot get the correct energy expression 
unless we integrate over the entire mass distribution; see \citet[Chapter 4]{shu:1992} and \citet{marcello:2012}. 
However, this approach is still valid when considering energy transfer between the gas energy and a  
potential field, if the change in energy is explicitly local. In the case of gravity, the mass flux
that generates a change in energy is local, but the change in energy is stored in the entire gravitational 
field, and is only captured by a global integral because of the long-range nature of the potential.
...
is displayed in Figure \autoref{fig:sponge}. We set $\Delta
...
the global timestep by up to an order of magnitude.  This means, however, 
that while the sponge is applied we should be cautious of imputing any physical 
...
refinement that jumps by a factor of four. However, even then there is a slight uptick
