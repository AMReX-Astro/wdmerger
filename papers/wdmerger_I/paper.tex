...
$\Delta x = 25\ \text{km}$. We chose this value because no simulation
...
refinement strategy: all zones within the Roche radius of each star are tagged for maximal 
refinement. We also ensure that the outer part of the domain is never tagged for refinement.
In future work we will add criteria that tag for refinement the gas between the stars, which is
expected to feature nuclear burning.
