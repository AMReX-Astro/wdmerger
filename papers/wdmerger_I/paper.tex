...
problem and we discuss our implementation here. We present an 
extensive set of test problems demonstrating the extent to which our software can sufficiently
...
exactly tracked. At each zone face in a given timestep, there is some transfer of mass $\Delta \rho \Delta V$ between 
...
index $i-1/2$. Energy is transferred because an amount of mass $\Delta \rho_{i-1/2} \Delta V$ moves from 
...
Note that while total energy is conserved, the gas energy is not because \autoref{eq:grav_energy_conservation_update} does not have the 
...
The change in the density $\Delta \rho_{f}$ is proportional to the mass flux through the surface with area $\Delta A$:
\begin{equation}
...
at the end of the timestep). Using this conservative energy formulation is very important for simulations where the 
potential can change substantially over the course of the evolution, as massive amounts of energy can be lost. 
The standard approach is therefore unacceptable in the case of a problem like white dwarf mergers, and 
...
cell center. Analytically this form expresses the same core idea as \autoref{eq:grav_energy_conservation_update} 
via the work-energy theorem, but a major flaw is that it evaluates the energy change at the cell center 
when in fact the mass transfer is happening at the cell edges. However, there is a more fundamental issue. 
As observed by \citet[Chapter 4]{shu:1992} and \citet{marcello:2012}, the energy source term $(-\rho \mathbf{u}\cdot\nabla\Phi)$ 
includes a contribution that is not localizable and reflects the global nature of the gravitational potential. 
It is only by integrating this source term over all space that the correct gravitational potential energy 
for the system emerges. Integrating over only one zone cannot yield the correct description of this global 
energy flow, and indeed therefore can never obtain the factor of 1/2 that arises from avoiding double-counting 
of the pairwise gravitational interactions. So while \autoref{eq:grav_energy_conservation_update} was correctly obtained 
by \cite{arepo} (whose work was the primary inspiration for this approach), their derivation that integrates the work 
done over the volume of a zone does not correctly account\footnote{They argued that this volume integral 
of the work can be converted into a surface integral by use of the divergence theorem (see their Equation 93). That 
application of the divergence theorem is mathematically incorrect, because their formulation of the integrand cannot actually 
be expressed only as the divergence of a vector field.} for this behavior. It is only by accounting for the global nature of the
potential that we can obtain the correct factor of $1/2$ and have a correct accounting of the energy within a zone.
The insight of applying the source term on the zone faces to conserve energy is still valid when 
considering energy transfer between the gas energy and a potential field if the change in energy 
is explicitly localizable. Indeed, we employ this formulation in \autoref{sec:rotation} for the case of the rotational potential.
...
very high orders the approximation breaks down, as seen in \autoref{fig:bc_comparison}. 
...
will be similar in size to the frequency of the orbital motion, which is in the range 
10-100 mHz for our problem. This is well outside the range of currently existing 
gravitational wave detectors but is very well suited for proposed space-based detectors such as eLISA.
