...
We employ a ``sponge'' similar to that described by \citet{maestro3}
for the outer regions of the computational domain. After the
hydrodynamics update, we apply a damping force to the momentum
equation as follows:
\begin{equation}
...
\end{equation}
For the simulations in this paper we set $r_S$ to be 75\% of the \MarginPar{maybe a plot of the sponge profile?}
distance from the origin to the domain boundaries, and $\Delta r_S$ so
that the sponge extends another 10\% of that distance. We set $\Delta
t_S = 0.01$ s, which is of the same order as the as the CFL timestep
for typical problem setups. We find that this sponge is necessary
especially in the rotating reference frame, as standing instabilities
can create very large velocities in the ambient fluid that drag down
the global timestep by up to an order of magnitude.  However, this
provides another reason to distrust the dynamics of the low-density
fluid outside of the stellar orbits. \MarginPar{I am not sure I understand 
this statement}
...
one-dimensional model. However, the magnitude of this change becomes
smaller with resolution. The star is only approximately in equilibrium
when the coarse grid of $256^3$ zones has a level of refinement that
jumps by a factor of four. However, even then there is a slight uptick
in the size toward the event, implying that the numerical stability is
not guaranteed for arbitrarily long timescales. This result suggests
that when doing the equilibrium initial models that will former the
basis of later calculations, we should carefully monitor the evolution
of the stars before applying any artificial damping to cause the
merger, to ensure that the merger is due to this applied force and not
the intrinsic numerical instability of the stars. \MarginPar{you never referenced figure 4}
...
the initial flow. However, at low resolution the tested grid algorithm
...
and disrupts the flow at high resolution.  However, we strongly
...
times. As observed by \citeauthor{mcnally:2012}, this is because the
...
To see this effect in action for a stellar simulation, we repeated the
test of \autoref{sec:HSE} with a bulk velocity on the grid. We chose a
velocity of $2.56 \times 10^{8}\ \text{cm s}^{-1}$.  \MarginPar{Approx
  what Mach number is this?}  {For context, this is comparable to the
  orbital velocities of the stars in \autoref{sec:kepler}.  This test
  ...
  computed with respect to the stellar center.  \MarginPar{are you
    sure that the single WD left quiet in the {\em corner} of the
    bigger domain is as stable as it was in the earlier test?}
...
noise.  \MarginPar{I really think that we should be able to do better here---this is where I'd like to see ppm\_type = 2, or better hydro-gravity coupling (eventually)}
...
Another that we consider is the energy conservation of the
...
which we will shortern to {\tt gs} for the present discussion. ${\tt
  gs} = 1$ is the standard cell-centered source term for
gravity. ${\tt gs} = 2$ is a slight variation on this scheme that
determines the value of the energy source term after the momentum
source term has been applied; the former approach uses the uncorrected
momenta in calculating $\rho \mathbf{u} g$, which we have found to be
more accurate. \MarginPar{which is more accurate?  Also, the ``standard approach'' is {\tt gs = 2} --- this is what is written, for exmaple, in the PPM paper}${\tt gs} = 3$ is entirely different: after calculating
the new momenta, we reset the total energy to be equal to the internal
energy plus the kinetic energy. This has the virtue of ensuring that
...
stable. However, the cost is that the oscillations about this
equilibrium are slightly larger than in the inertial frame. The
unequal mass case displays this effect starkly: while the average
distance is not declining, there are huge oscillations about this
...
frame. \MarginPar{this is interesting---has anyone else seen this?}
...
Meanwhile, angular momentum conservation is not quite as good.\MarginPar{``as good'' as what?}  This is
...
recently switched to a tiling approach \MarginPar{ref Weiqun's paper}, where an OpenMP region is
generated at the start of the hydrodynamics routine and the individual
threads separately work on different partitions of each box. This
...
$128^3$ zones through $2048^3$ zones. \MarginPar{is this really weak scaling?} The number of processors was
...
at very large processor counts when self-gravity is considered. \MarginPar{does this use the new MG initialization of $\Phi$ from the old step?}
