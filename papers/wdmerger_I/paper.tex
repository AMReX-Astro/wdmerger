...
(We note in passing too that although \cite{jiang:2013} claim their approach requires a Poisson 
...
reference state be as in Eq.~\autoref{eq:refchoice}.
Ignoring flattening, if there are no waves moving toward our
interface, then Eq.~\autoref{eq:ppmstatel} reduces to:
\begin{equation}
...
added in Eq.~\autoref{eq:ppmstatel}, but our choice of reference state makes
...
in Eq.~\autoref{eq:refchoice}, and consider a quantity $\xi$ which should only
