...
As observed by \cite{arepo}, when examining Equation \ref{eq:mass_transfer} one can see the relationship 
...
It is worth pointing out that while Equation \ref{eq:grav_energy_conservation_update} was correctly obtained 
...
Indeed, we will do so for the rotational potential in Section \ref{sec:rotation}, where the 
...
Section \ref{sec:gravity_hydro_coupling}. We again continue to evolve explicitly an equation for 
...
\begin{figure*}[ht]
  ...
  \caption{2D Kelvin-Helmholtz instability test at $t = 2.0$ for the initial 
    conditions given by Equations \ref{eq:kh_ic_b_ramp} and \ref{eq:kh_ic_b}. 
    ...
    \citet{robertson:2010}, Figure 7. \label{fig:kh_p2}}
\end{figure*}
...
\begin{figure*}[ht]
  ...
  \caption{2D Kelvin-Helmholtz instability test at $t = 2.0$ for the initial 
    conditions given by Equations \ref{eq:kh_p3_rho} through \ref{eq:kh_p3_vy}. 
    The meaning of the rows and columns is the same as in Figure \ref{fig:kh_p2}.
    \label{fig:kh_p3}}
\end{figure*}
...
for the normal resolutions and all velocities in Figure \ref{fig:kh_p2}.
...
by examining the first row of Figure \ref{fig:kh_p2}.

\begin{figure*}[ht]
  \includegraphics[trim = 0in 1.25in 0.25in 1.25in, clip, scale=0.65]{{{plots/kh_p3_high_res_collage}}}
  \caption{Time series of the Kelvin-Helmholtz problem proposed by \citet{mcnally:2012}
...
systems is enough to examine their effects. In Section \ref{sec:gravity_boundary_conditions},
...
The results are shown in Figure \ref{fig:gravity_bcs}. Terms in the boundary potential 
...
Another that we consider is the energy conservation of the system. Recalling Section \ref{sec:gravity_hydro_coupling}, 
...
after a single orbit are seen in the first column of Table \ref{table:sources}. The first two versions give reasonable and 
...
In Table \ref{table:sources} we show also the effects on energy conservation of using the inertial reference frame. 
...
but evolve them for 25 orbital periods. In Figure \ref{fig:circular_orbit_comparison} we illustrate the evolution of these systems 
...
in the inertial frame for energy conservation (Figure \ref{fig:energy_conservation_equal}), and the unequal mass 
system in the rotating frame for angular momentum conservation (Figure \ref {fig:angular_momentum_conservation_unequal}). 
...
\begin{figure}
  \centering
  \includegraphics[scale=0.45]{plots/equal_energy_rot_0.eps}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.45]{plots/unequal_angular_momentum_rot_1.eps}
...
the parabolic profiles \citep{ppm}; see Section \ref{sec:Hydrodynamics} for a brief 
...
reference state be as in Eq.~\ref{eq:refchoice}.
