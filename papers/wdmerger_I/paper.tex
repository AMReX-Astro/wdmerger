...
overflowing its Roche lobe.\MarginPar{ref?} \cite{saionomoto:1985} tracked the
...
found that a detonation occurred in a similarly massive system.\MarginPar{``similarly massive system'' repeated twice in succession} Notably,
...
The Euler equations for hydrodynamics (in the absence of source terms) in conservative form are: \MarginPar{we usually use $U$ for the velocity vector, with $U = (u, v, w)$}
\begin{align}
...
reset the characteristics \MarginPar{``characteristics'' has a very precise meaning for hyperbolic systems, use a different word} of the affected zone. We choose the latter
...
$10^{5}\ \text{K}$ \MarginPar{do we really need it this cold?  I thought we were doing $10^6$~K} for our stellar simulations). We then recompute 
...
the end to synchronize the various levels. Generally \MarginPar{why ``Generally''?  Is there ever as case where the subcycled step isn't the jump in refinement?} the number of
...
There are two ways that these source terms enter the hydrodynamics update. \MarginPar{``hydrodynamics update'' appears twice in succession}
...
source term evaluated at time-level $n$. \MarginPar{we trace, so it's not just the cell-centered source term} The details of this are described in 
...
self-gravity. \MarginPar{also try for Castro core-collapse sims---maybe reference a paper on each subject from castro} However, for a problem like that of a binary star system
...
The resolution study for this case is also
plotted in \autoref{fig:gravity_convergence}. \MarginPar{explain that this is the ``normalized sphere'' in the figure} We still obtain
...
$v_x^{n+1}$ \MarginPar{we are using $u, v, w$ below, and is more consistent with other papers / User's Guide} depends on the value of $v_y^{n+1}$, and vice versa. If we fix the value of 
...
quantities back to the inertial reference frame when reporting them. In particular,
for every zone we adjust the momenta and kinetic energy to account for rotation at 
frequency $\omega$. When plotting the locations of 
stars on the grid, we rotate them by an angle corresponding to the amount of time that has 
passed since the simulation started. \MarginPar{show the transforms.  Also, be more explicit---is this just done for the plotfiles?}
...
\MarginPar{use subsections to break the distinct topics apart in this section---it all runs together}
...
together to obtain the full zone's characteristics \MarginPar{use ``state'' instead of ``characteristics''}. This
...
We employ a ``sponge'' similar to that described by \citet{maestro3} for the outer regions of the 
computational domain. After the hydrodynamics update, we apply a damping force to the momentum 
equation as follows:
\begin{equation}
