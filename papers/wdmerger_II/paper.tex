...
some simulations at high resolution we were mainly interested in seeing when the
detonation happened, and we terminated the simulation at that point to save
computational cycles. We show nucleosynthesis results only for those simulations
that had completed the nuclear burning.}. At
...
collision time, $t_{\rm c}$, as the first time when the maximum density on the grid reaches
...
domain. We define the detonation time, $t_{\rm d}$, as the time when the temperature
...
refinement is used. For this configuration, the collision time is $t_{\rm c} = 6.47\ \text{s}$,
...
At high resolution the collision time is $t_{\rm c} = 6.41\ \text{s}$, which is similar to the
