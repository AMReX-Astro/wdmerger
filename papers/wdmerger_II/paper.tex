...
this nickel mass. Plotfiles of a few relevant quantities (density, temperature, velocity,
pressure, sound speed, and the ratio $\tau_s / \tau_e$) are created every 0.01 seconds.
The timesteps are limited to ensure that every multiple of this plotfile interval is
reached exactly, which implies an effective maximum timestep of 0.01 seconds.
...
resolutions. In each case we ran the simulation to the point when the density
at the collision point reached $10^7\ \text{g / cm}^{-3}$, which we will informally
call the collision time, $t_{\rm c}$. The collision time is about $t_{\rm c} = 6.85\ \text{s}$
