...
white dwarf collisions feature sharply different gravitational wave signatures, and
hence may be interesting targets for observation for future low frequency gravitational
wave detectors \citep{eLISA}. The main caveat for these events compared to events
...
the representative case of \autoref{sec:parameters}. We use the convention that the
...
with the gravitational wave signature predicted by \cite{loren-aguilar:2009:collisions},
though because they used an unequal mass pair the results do not match exactly.

\begin{figure}
  \centering
  \includegraphics[scale=0.425]{plots/gw_signal}
  \caption{Gravitational wave strain for the head-on collision of two $0.64\ \msolar$
