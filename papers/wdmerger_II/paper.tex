...
(A related approach presented in \citet{kushnir:2013} is to limit the magnitude
of the energy release in any given integration by scaling all the reaction rates.)
At the start of each advance, we limit the size of the timestep to be the smaller
of the minimum hydro timestep (limited by the CFL condition), and the minimum burning
timestep. In a given zone, the burning timestep is defined as:
\begin{equation}
  \Delta t_{\text{burning}} = f_b\, \frac{e}{\dot{e}}.
\end{equation}
...
update, divided by the length of the burning timestep. The factor $f_b$ then
...
timestep to timestep, which is not always the case). By making $f_b$ smaller,
we can control the magnitude of the decoupling between the reactions and the hydro.
A typical choice in the literature is $f_b \sim 0.3$. This timestep limiting has
now been implemented in \castro\ and the factor $f_b$ can be set at runtime.
