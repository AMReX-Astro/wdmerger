...
denominator in Equation \ref{eq:burning_mode_4} with the change in 
...
used the method of spectral deferred corrections \cite{SDC} to
...
Finally, we return to a point we hinted at above. The timestep
...
quite radically during periods of runaway burning when
the rate of change of these quantities is highly nonlinear.
We do not want to neglect the errors caused by this approximation
...
\castro. The retry option works by first saving a copy of the
full state, and then doing the advance. We then explicitly
check whether the timestep criteria were violated during the advance,
using the time-centered energy and mass fractions and a
time-centered differencing of the old and new states to
calculate the evolution timescale. If so, we reject this
advance and restore the original state data. We then perform
a subcycled update, with the maximum timestep chosen so that
the criterion is satisfied in each subcycle. Note that this
occurs entirely on a single level and is separate from the
subcycling that is used by \boxlib\ to advance the different
AMR levels in time; the full AMR algorithm does not ever
know that the retry subcycling took place. This can help
substantially to mollify the problem described above, where
the time derivative estimation methods proposed were not
sufficient to obtain the desired limiting. Thus we strongly
recommend that hydrodynamics codes implement such an approach,
which ensures that the coupling actually does stay at the
level requested. The most significant drawback is that saving
a copy of the old state effectively doubles the memory footprint
of the code, so this option should be disabled when not needed.
However, it may be crucial in determining the evolution of
systems such as white dwarf collisions, where even a short
period of incorrect advances can significantly pollute the
final answer, and the additional memory expense is justified
by obtaining a more accurate solution.
...
the problem unfeasible, given current computational resources.
