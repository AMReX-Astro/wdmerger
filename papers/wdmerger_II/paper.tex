...
first reaches $5 \times 10^9\ \text{K}$.
...
Except for the discussion later about the suppressed burn mode, the simulations
shown in this work all use the self-heating mode. The hydrostatic and hybrid burns
are only a slight variation on the self-heating burn in practice for this problem.
...
effects it has on the outcome of the simulation.
...
In \autoref{sec:burner} we observed that there is an alternative to the traditional
self-heating approach in a burn, the ``suppressed'' burning mode. This alternative
is relevant to a collision problem because, as pointed out by \cite{raskin:2010}
and \cite{kushnir:2013}, it is possible for a typical zone in our simulation to
release a very large amount of energy in a burn before it is cooled
off by a subsequent hydrodynamic expansion step, leading to the numerically unstable burning
problem described in \autoref{sec:unstable_burning}. However, the suppressed burning mode
(modeled after \citet{kushnir:2013}) produces only about
...
simulation to the point at which unstable burning was approached. Then we stopped
