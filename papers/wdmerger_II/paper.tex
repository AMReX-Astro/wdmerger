...
collision time, $t_c$, as the first time when the maximum density on the grid reaches
$5 \times 10^6\ \text{g / cm}^3$. This is slightly larger than the initial maximum
density of the two WDs, and is reached only when the WDs meet at the center of the
domain. We define the detonation time, $t_d$, as the time when the temperature
first reaches $4 \times 10^9\ \text{K}$.
...
refinement is used. For this configuration, the collision time is $t_c = 6.47\ \text{s}$,
and the detonation time is $t_d = 7.43\ \text{s}$. The peak density at the detonation time
...
size of the hotspot stays the same. One cannot appeal to the numerical instability
criterion alone to make a decision in this context.

Furthermore, we find that \textit{all} of the detonations we have seen at low
and moderate resolution inevitably violate the stability criterion. This is true
even if the prompt detonation is suppressed: the delayed detonation also is
numerically unstable. For comparison purposes, we developed an option for our
burner to use a ``suppressed'' burning mode. In a suppressed burn, we limit
...
criterion provided by \cite{garg:2017} indicates that typical critical
radii for detonation of equal C/O material are in the range 1-10 km.
...
gradient will be resolved. We have developed an AMR criterion
to help achieve this resolution: we tag on regions suggested by
the burning stability criterion, \autoref{eq:burning_limiter_2}.
The original motivation for this choice was that by lowering
$\Delta x$ and thus the sound-crossing time, we could reach
the point where the detonation was occurring in a numerically stable
region. This goal was not realized due to the reasons discussed above.
Instead, we simply use it as a guide for where to refine. We tag
for refinement all zones with $10^{-6} \leq f_{s} \leq 1$, where
$f_{s}$ is the ratio of the sound crossing time to the energy injection
time. We do not target zones for refinement with $f_{s} > 1$ since
at that point a detonation is locked in. We refine up to a specified
maximum resolution ratio relative to the coarse grid.
...
The collision time is about $t_{\rm c} = 6.85\ \text{s}$
at high resolution. At low resolution, the simulation is slightly different because
of the distortion imposed by a rectangular grid representing initially spherical stars,
so the collision time is slightly later. Then, we restarted the run from this checkpoint
and reset the time to zero (such that $t_{\rm c} = 0$). The new simulation runs until about
$t = 2.15\ \text{s}$, so that the full 9 seconds are simulated. During this second phase
the detonation occurs. An estimate of the detonation time, $t_{\rm d}$, that serves well
is the time when the peak density on the grid first reaches $5 \times 10^9\ \text{K}$.
We measure the detonation time relative to $t_{\rm c}$. Approximately speaking, the later
$t_{\rm d}$ is, the more nickel is produced. As time passes, more white dwarf material
slams into the center, compressing and heating up the gas. Both effects are favorable
to production of nickel. So, a later $t_{\rm d}$ implies that more material can be converted
to nickel. Similarly, the speed of the detonation matters. A slower speed detonation prolongs
the collision event, allowing more gas to be processed into nickel.
